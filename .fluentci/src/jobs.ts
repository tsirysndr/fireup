...
export const build = async (src = ".") => {
  ...
  const ctr = dag
    ...
    .withExec(["sh", "-c", "cp target/${TARGET}/release/fireup ."])
    .withExec([
      "sh",
      "-c",
      "tar czvf /assets/fireup_${TAG}_${TARGET}.tar.gz fireup README.md LICENSE",
    ])
    .withExec([
      "sh",
      "-c",
      "shasum -a 256 /assets/fireup_${TAG}_${TARGET}.tar.gz > /assets/fireup_${TAG}_${TARGET}.tar.gz.sha256",
    ])
    .withExec(["sh", "-c", "cp /assets/fireup_${TAG}_${TARGET}.tar.gz ."])
    .withExec([
      "sh",
      "-c",
      "cp /assets/fireup_${TAG}_${TARGET}.tar.gz.sha256 .",
    ]);

  const exe = await ctr.file(
    `/app/fireup_${Deno.env.get("TAG")}_${Deno.env.get("TARGET")}.tar.gz`
  );
  await exe.export(
    `./fireup_${Deno.env.get("TAG")}_${Deno.env.get("TARGET")}.tar.gz`
  );

  const sha = await ctr.file(
    `/app/fireup_${Deno.env.get("TAG")}_${Deno.env.get("TARGET")}.tar.gz.sha256`
  );
  await sha.export(
    `./fireup_${Deno.env.get("TAG")}_${Deno.env.get("TARGET")}.tar.gz.sha256`
  );
  return ctr.stdout();
};
